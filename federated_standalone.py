...
with st.sidebar:
    ...
    if available_port:
        st.success(f"✅ Port {available_port} is available")
        st.info(f"🌐 Will run on: http://localhost:{available_port}")
        
        # Auto-open browser button
        if st.button("🌐 Open Browser", type="secondary"):
            open_browser(f"http://localhost:{available_port}")
    else:
        st.error("❌ No ports available (8501-8600)")
        st.info("💡 Try closing other applications or restart your computer")
...
<p>🔗 Federated Learning Platform - Standalone Version</p>
