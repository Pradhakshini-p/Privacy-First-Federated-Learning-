...
class DiabetesMLP(nn.Module):
    ...
    Architecture optimized for healthcare tabular data.
    """

    def __init__(self, input_dim=8, hidden_dims=[64, 32, 16], output_dim=2, dropout_rate=0.3):
...
class DiabetesCNN(nn.Module):
    """
    1D CNN for Diabetes Prediction.
    Useful when treating healthcare features as sequential data.
    """
...
def create_model(model_type="mlp", input_dim=8, **kwargs):
    """
    ...
    if model_type.lower() == "mlp":
        return DiabetesMLP(input_dim=input_dim, **kwargs)
    elif model_type.lower() == "cnn":
        return DiabetesCNN(input_dim=input_dim, **kwargs)
    else:
...
def test_model():
    ...
    mlp_model = DiabetesMLP(input_dim=8).to(device)
    dummy_input = torch.randn(32, 8).to(device)
    ...
    # Test CNN
    print("\nTesting CNN model...")
    cnn_model = DiabetesCNN(input_dim=8).to(device)
    ...
    print("\n✅ Diabetes prediction models are working correctly!")
