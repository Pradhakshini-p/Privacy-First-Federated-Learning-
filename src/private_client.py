#!/usr/bin/env python3
"""
Enhanced Client with Differential Privacy
Integrates Opacus for privacy-preserving federated learning
"""
...
class PrivateFraudDetectionClient:
    def __init__(self, client_id: int, silo_id: str = None, model_type="mlp", 
                 privacy_enabled=True, noise_multiplier=1.0, max_grad_norm=1.0, delta=1e-5):
        ...
        logger.info(f"🔒 Private Client {self.client_id} initialized")
        ...
        logger.info(f"🛡️ Privacy: {'ENABLED' if self.privacy_enabled else 'DISABLED'}")
        if self.privacy_enabled:
    ...
    def fit(self, parameters: List[np.ndarray], config: Dict[str, fl.common.Scalar]) -> Tuple[List[np.ndarray], int, Dict[str, fl.common.Scalar]]:
        ...
        logger.info(f"🏋️ Private Client {self.client_id}: Starting training for {local_epochs} epochs...")
        ...
        for epoch in range(local_epochs):
            ...
            for batch_idx, (data, target) in enumerate(self.train_loader):
                ...
                if batch_idx % 10 == 0:
                    logger.info(f"📊 Private Client {self.client_id}: Epoch {epoch+1}, Batch {batch_idx}, Loss: {loss.item():.4f}")
            
            epoch_accuracy = epoch_correct / epoch_total
            logger.info(f"✅ Private Client {self.client_id}: Epoch {epoch+1} completed - Loss: {epoch_loss/len(self.train_loader):.4f}, Accuracy: {epoch_accuracy:.4f}")
        ...
        if self.privacy_enabled and self.privacy_engine:
            try:
                ...
                logger.info(f"🛡️ Privacy budget spent: ε = {epsilon:.4f}")
            except Exception as e:
                logger.warning(f"Could not calculate epsilon: {e}")
                epsilon = 0.0
        ...
        metrics = {
            ...
            "noise_multiplier": self.noise_multiplier if self.privacy_enabled else 0.0,
        }
        
        logger.info(f"🎯 Private Client {self.client_id}: Training completed - Val Loss: {val_loss:.4f}, Val Accuracy: {val_accuracy:.4f}")
    ...
    def evaluate(self, parameters: List[np.ndarray], config: Dict[str, fl.common.Scalar]) -> Tuple[float, int, Dict[str, fl.common.Scalar]]:
        ...
        logger.info(f"🔍 Private Client {self.client_id}: Starting evaluation...")
        ...
        metrics = {
            ...
            "client_id": self.client_id,
            "privacy_enabled": self.privacy_enabled,
            "latest_epsilon": self.epsilon_history[-1] if self.epsilon_history else 0.0,
        }
        
        logger.info(f"📈 Private Client {self.client_id}: Evaluation completed - Loss: {val_loss:.4f}, Accuracy: {val_accuracy:.4f}")
        
        return val_loss, len(self.val_loader.dataset), metrics

class PrivateFlowerClient(fl.client.NumPyClient):
    def __init__(self, client_id: int, silo_id: str = None, model_type="mlp", 
                 privacy_enabled=True, noise_multiplier=1.0, max_grad_norm=1.0):
        self.private_client = PrivateFraudDetectionClient(
            client_id, silo_id, model_type, privacy_enabled, noise_multiplier, max_grad_norm
        )
    
    def get_parameters(self, config: Dict[str, fl.common.Scalar]) -> List[np.ndarray]:
        return self.private_client.get_parameters(config)
    
    def fit(self, parameters: List[np.ndarray], config: Dict[str, fl.common.Scalar]) -> Tuple[List[np.ndarray], int, Dict[str, fl.common.Scalar]]:
        return self.private_client.fit(parameters, config)
    
    def evaluate(self, parameters: List[np.ndarray], config: Dict[str, fl.common.Scalar]) -> Tuple[float, int, Dict[str, fl.common.Scalar]]:
        return self.private_client.evaluate(parameters, config)

def start_private_client(client_id: int, silo_id: str = None, model_type="mlp", 
                        server_address="localhost:8080", privacy_enabled=True, 
                        noise_multiplier=1.0, max_grad_norm=1.0):
    """Start a private Flower client."""
    logger.info(f"🚀 Starting private client {client_id}...")
...
def main():
    parser = argparse.ArgumentParser(description="Start Private Flower Fraud Detection Client")
    ...
    start_private_client(
        args.client_id, silo_id, args.model, args.server, 
        not args.no_privacy, args.noise_multiplier, args.max_grad_norm
    )
